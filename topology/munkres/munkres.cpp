...
Munkres::step2(void) {
  ...
  int k = matrix.rows() < matrix.cols() ? matrix.rows() : matrix.cols();
...
Munkres::step3(void) {
  ...
  */
	...
	std::priority_queue<std::pair<int,int>, std::vector<std::pair<int,int> >, PairCompare> pair_queue;
	for(int r = 0; r < matrix.rows(); r++)  {
		...
		for(int c = 0; c < matrix.cols(); c++)  {
			if(col_mask[c])
				continue;
			if(matrix(r,c) == 0)
				pair_queue.push(std::pair<int,int>(r,c));
		}
	}

	while(true)  {
		bool found_uncovered_entry = false;
		/*
		std::vector<int> uncovered_cols;
		for(int c = 0; c < matrix.cols(); c++)  {
			if(!col_mask[c])
				uncovered_cols.push_back(c);
		}
		for ( saverow = 0 ; saverow < matrix.rows() ; saverow++ )  {
			if ( row_mask[saverow] )
				continue;
			for (int c = 0 ; c < uncovered_cols.size() ; c++ )  {
				savecol = uncovered_cols[c];
				if ( matrix(saverow,savecol) == 0 )  {
					found_uncovered_entry = true;
		...
		*/
		...
		bool verify_found_uncovered = false;
		int verify_saverow = -1, verify_savecol = -1;
		while(pair_queue.size() > 0)  {
			std::pair<int,int> best_pair = pair_queue.top();
			if(row_mask[best_pair.first])
				pair_queue.pop();
			else  {
				verify_saverow = best_pair.first;
				verify_savecol = best_pair.second;
				mask_matrix(verify_saverow,verify_savecol) = PRIME;
				verify_found_uncovered = true;
				break;
			}
		}
		found_uncovered_entry = verify_found_uncovered;
		saverow = verify_saverow;
		savecol = verify_savecol;
			...
			if ( mask_matrix(saverow,ncol) == STAR ) {
				...
				for(int r = 0; r < matrix.rows(); r++)  {
					if(row_mask[r])
						continue;
					if(matrix(r,ncol) == 0)
						pair_queue.push(std::pair<int,int>(r,ncol));
				}
				break;
			}
		}

		if(uncovered_col) // repeat
			continue;
		else
...
void 
Munkres::solve(Eigen::MatrixXd &m) {
  ...
  mask_matrix = Eigen::MatrixXi::Zero(matrix.rows(),matrix.cols());
  ...
  while ( notdone ) {
    switch ( step ) {
      ...
      case 3:
        ...
        step = step3();
		  step3_timer.end();
		  //step3_timer.dump_time();
        break;
      case 4:
		  step4_timer.start();
        step = step4();
		  step4_timer.end();
		  //step4_timer.dump_time();
        break;
      case 5:
		  step5_timer.start();
        step = step5();
		  step5_timer.end();
		  //step5_timer.dump_time();
        break;
    }
  }
