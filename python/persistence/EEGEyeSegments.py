...
class EEGEyeSegments(Segments):
    """
    Segments generated from the EEG data contained in 
    http://suendermann.com/corpus/EEG_Eyes.arff.gz

    Each line after the line @data contains a list of 14 floating
    point values from EEG sensors and one 0,1 value corresponding if
    the eyes are open (0) or closed (1)

    config.data_index corresponds to the indices of the values in each
    line to be used, and the label is the 0,1 value.
    """
    ...
    def get_segment_filename(config, gz=True):
        ...
        return get_filename(config, fields, 'EEGEyeSegments', gz)
